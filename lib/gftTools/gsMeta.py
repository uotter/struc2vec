...
from lib.gftTools import metaInfo_pb2 as mtproto
from lib.gftTools import instStream_pb2 as instStream
import copy
import os
...
def slice_data_return_new(data, begin_time: pd.Timestamp, end_time: pd.Timestamp, pt_name):
    if isinstance(data, dict):
        ...
        for key, value in data.items():
            ret.update({key, slice_data_return_new(value, begin_time, end_time, pt_name)})
        return ret
    if isinstance(data, gftIO.GftTable):
...
class InstrInstanceDesc(DataframeDesc):
    ...
    def set_data(self, j_gid, input_lookback: LookbackStep, primary_t, col_metas, min_t, max_t, min_j_gid, max_j_gid,
        ...
        # print("(1)InstrInstanceDesc[set_data]column meta size:{0}".format(str(col_meta_size)) )
...
class ResampleDescGenerator(DescGenerator):
    ...
    def get_instr_desc(self, gid, input_list, inst_id=None):
        ...
        if resample_end_date > freq.max_t:
            ...
            max_t = gftIO.pd_dt_2_gs_day(freq.trade_calendar.index[freq.trade_calendar.index.get_slice_bound(gftIO.gs_day_2_pd_dt(resample_end_date), 'left', 'loc')])
...
def create_desc_generator_4_cpp():
    ...
    lead_code_meta = LeadDescGenerator()
    ...
    return empty_code_meta, pts_2nd_index, resample_code_meta, transform_2_scalar_code_meta, lead_code_meta, dict_meta, extract_meta, cpp_with_global_t, raise_dimension_gen
